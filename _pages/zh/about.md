...
**独立项目。** 手工 CoT 表面特征的跨领域行为。 [交互式 demo](https://chi-shan0707.github.io/code-not-text/demo/)

<details markdown="1">
<summary>概述</summary>

**问题。** 推理轨迹中的手工特征能否从数学、科学问答迁移到代码任务？

**方法。** 我使用 token 置信度、轨迹连续性、反思次数、新颖性等特征，比较它们在不同领域中的预测行为。
...
**Takeaway。** 这些特征在数学类推理轨迹中可能有用，但代码正确性更直接依赖可执行行为。我把它视为 measurement 问题，而不是普遍否定结论。
